...
async def generate_recommendations(recommendation_fetch_data: RecommendationFetchDTO):
    ...
    if (not recommendations) and (is_processing == False or is_processing is None):
        asyncio.create_task(recommednations.generate_qloo_powered_recommendations(
    ...
    recommendations = await get_recommendations_by_details(details=details, page=1, wait = False)
    return recommendations

async def get_recommendations_by_details(details: dict, page=1 , wait=True):
    """
    ...
    while not recommendations:
        ...
        if wait: # If wait is True, we will keep checking for recommendations until we get some or reach a limit
            if trials >= 3:
                # Confirm is processing status is True to keep waiting else break
                if is_processing == False or is_processing is None:
                    break
        else:
            # If wait is False, we will not wait for recommendations and just return what we have after 1
            if trials >= 1:
                if is_processing == False or is_processing is None:
                    break
