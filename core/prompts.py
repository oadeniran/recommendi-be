...
**If the message is unrelated to the selected category, set is_valid = false and return empty string for keyword, generic_term, and location**
    ...
    Its possible that users can want movies based on books but thw wording here is the KEY way to determine if user is asking for a book or a movie
     ...
     - Eg. Selected category is places and user says "Where can I watch a movies in Lagos?" is_valid is True here because user is asking for a place to watch movies, not just asking for movies. 
     IMPORTANT: If the user message refrences a physical location (Country, City, State, etc.) and something else related to the selected category, then is_valid MUST BE true
